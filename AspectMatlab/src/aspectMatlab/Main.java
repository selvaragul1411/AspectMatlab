...
public class Main
{
	...
	public static void main(String[] args)
	{
		...
		String outPath = "weaved"; //location where woven code will be placed
		
		boolean isEntryPoint = false;
		boolean isMatlabConvert = true;
		...
		for(int i=0; i<args.length; i++){
			...
			if(file.compareTo("-version") == 0){ //version
				...
				isEntryPoint = true;
				continue;
			} else if(file.compareTo("-n") == 0){ //matlab translation
			...
			if(isMatlabConvert && !isAspect(file)) {
				try{
					fileReader = new FileReader( file );

				
				System.err.println("Translating "+file+" to Natlab");
				fileReader = natlab.Parse.translateFile( file, errors ).getReader();
				...
				int character;		
				File f = new File("nattemp"+file);
				srcname = "nattemp" + file;
				f.createNewFile();
				f.deleteOnExit();
	...
	//Returns true if the filename corresponds to an aspect, false otherwise.
	private static boolean isAspect(String file){
		...
		try{
			...
			line = br.readLine();
			isAspect = line.startsWith("aspect");
	...
	//prepares annotations for parsing
	private static void prepareAnnotations(String file){
	try{		
		FileInputStream fstream = new FileInputStream(file);
  		DataInputStream in = new DataInputStream(fstream);
  		BufferedReader br = new BufferedReader(new InputStreamReader(in));
		String strLine = "";
		File f = new File("anntemp"+file);
		...
		while((strLine = br.readLine()) != null){
			if(strLine.startsWith("%@")){
				strLine = "AMannotation " + strLine.substring(2);
			}
		...
		System.err.println("File "+file+" can not be opened!\nAborting");
	...
	private static void displayUsage()
	{
		...
		System.out.println("\t-n: to skip translation of standard MATLAB to Natlab");
