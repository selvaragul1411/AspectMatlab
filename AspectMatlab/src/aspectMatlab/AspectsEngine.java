...
public class AspectsEngine {
	...
	static public Map<String, Expr> patternsList = new HashMap<String, Expr>();
	...
	static final public String TYPE = "istype";
	static final public String ANNOTE = "annote";
	...
	*/
	...
	public static void printAST(ASTNode node, int indent){
		//Class<?> enclosingClass = node.getClass().getEnclosingClass();
		...
		if(node instanceof ast.Name){
			...
			System.out.println(node.getClass().getName() + " : " + ((Name)node).getID() + " : " + node.getNumChild());
		}else{
	...
	{ 	
		...
		for(Patterns patterns : aspect.getPatterns())
			for(Pattern pattern : patterns.getPatterns())
			{
				...
				patternsList.put(pattern.getName(), pattern.getPD());
			}
	...
	public static void generateCorrespondingStmt(Expr pe) {
		if(pe.getWeavability() && !pe.getCFStmtDone()) {
			...
			for(Expr pat : patternsList.values()) {
				if(pat.ShadowMatch(target, GETORCALLSIMPLE, -1, pe)) {
					...
					//Need to handle Short Circuit Statements
					if(pe instanceof ShortCircuitAndExpr || pe instanceof ShortCircuitOrExpr)
						return;
	...
	public static int simplifyExpr(Expr current,ast.List<Stmt> stmtList,int stmtPos){
		...
		//Don't simplify short circuit
		//if(current instanceof ShortCircuitAndExpr || current instanceof ShortCircuitOrExpr){
		//	return 1;
		//}		
		...
		}else if(parentNode instanceof ShortCircuitAndExpr || parentNode instanceof ShortCircuitOrExpr){
			return 1;
	...
	public static void transformBinaryExpr(BinaryExpr current,ast.List<Stmt> stmtList,int stmtPos){
		...
		for(int j=0; j<actionsList.size(); j++){
			ActionInfo act = actionsList.get(j);
			 ...
			 if(patternsList.containsKey(act.getPattern())){
				 
				String unparsedClass = current.getClass().toString();
			 	String parsedClass = unparsedClass.replaceAll("class ast.",""); 
			 	
			 	
			 	if(patternsList.get(act.getPattern()).ShadowMatch(parsedClass,OPERATORS, 2 , current)){
	...
	public static void weaveStmts(ast.List<Stmt> stmts)
	{
		...
		for(int s=0; s<stmtCount; s++)
		{
			...
			} else if(stmt instanceof AnnotationStmt){
				AnnotationStmt annotation = ((AnnotationStmt) stmt)	;		
				//System.out.println("ANNOTATION " + (annotation.getName()));
				
				int count = weaveAnnotation(annotation,stmts,s);
				s += count;
				stmtCount += count;
			}else{
				stmt.aspectsWeave();
	...
	//convert annotation arrays to cell arrays
	private static CellArrayExpr annoArrayToCellArray(AnnoArray anno){
		CellArrayExpr ca = new CellArrayExpr();
		Row row = new Row();
		
		//for each element, check what type of argument it is, cast it then put it into a row, which will be added to cell array
		for(int i = 0; i<anno.getNumArrayElement() ;i++){
			AnnoArgument aa = anno.getArrayElement(i);
			if(aa instanceof AnnoName){
	
				row.addElement(new NameExpr(new Name(((AnnoName)aa).getID())));

			}
					
			if(aa instanceof AnnoFPLiteralExpr){
				
				row.addElement(new FPLiteralExpr((((AnnoFPLiteralExpr)aa).getValue())));

			}
			if(aa instanceof AnnoIntLiteralExpr){
				row.addElement(new IntLiteralExpr((((AnnoIntLiteralExpr)aa).getValue())));
			}
			if(aa instanceof AnnoStringLiteralExpr){
			
				row.addElement(new StringLiteralExpr(((AnnoStringLiteralExpr)aa).getValue()));
			}
			
			//recursive call for nested annoarrays
			if(aa instanceof AnnoArray){
				row.addElement(annoArrayToCellArray((AnnoArray)aa));
			}		
		}
		ca.addRow(row);
		return ca;
	...
	//weaving annotations
	private static int weaveAnnotation(AnnotationStmt stmt, ast.List<Stmt> stmts,int s){
		int bcount = 0;
		int tcount = 0;
		int acount = 0;
		for(int j=0; j<actionsList.size(); j++)
			{
				...
				//first build cell array for arguments, converting annotation args to regular args
				Iterator<AnnoArgument> itr = stmt.getAnnoArgumentList().iterator() ;
				CellArrayExpr args = new CellArrayExpr();
				CellArrayExpr rep = new CellArrayExpr();
				Row row = new Row();
				Row rrow = new Row();
				
				while(itr.hasNext()){
					AnnoArgument arg = itr.next();
					if(arg instanceof AnnoName){
						//System.out.print("  " + (((AnnoName)arg).getID()));
						row.addElement(new NameExpr(new Name(((AnnoName)arg).getID())));
						rrow.addElement(new StringLiteralExpr((((AnnoName)arg).getID())));
					}
					
					if(arg instanceof AnnoFPLiteralExpr){
						//System.out.print("  " + (((AnnoFPLiteralExpr)arg).getValue()));
						row.addElement(new FPLiteralExpr((((AnnoFPLiteralExpr)arg).getValue())));
						rrow.addElement(new FPLiteralExpr((((AnnoFPLiteralExpr)arg).getValue())));
					}
					if(arg instanceof AnnoIntLiteralExpr){
						//System.out.print("  " + (((AnnoIntLiteralExpr)arg).getValue()));
						row.addElement(new IntLiteralExpr((((AnnoIntLiteralExpr)arg).getValue())));
						rrow.addElement(new IntLiteralExpr((((AnnoIntLiteralExpr)arg).getValue())));
					}
					if(arg instanceof AnnoStringLiteralExpr){
						//System.out.print("  " + (((AnnoStringLiteralExpr)arg).getValue()));
						row.addElement(new StringLiteralExpr(((AnnoStringLiteralExpr)arg).getValue()));
						rrow.addElement(new StringLiteralExpr(((AnnoStringLiteralExpr)arg).getValue()));
					}
					if(arg instanceof AnnoArray){
						row.addElement(annoArrayToCellArray((AnnoArray)arg));
						rrow.addElement(annoArrayToCellArray((AnnoArray)arg));
					}
				...
				args.addRow(row);
				rep.addRow(rrow);
				//add stmts for cell array creation
				Stmt argstmt = new AssignStmt(new NameExpr(new Name("args")),args);
				Stmt repstmt = new AssignStmt(new NameExpr(new Name("rep")),rep);
			if(patternsList.containsKey(act.getPattern())
				&& patternsList.get(act.getPattern()).ShadowMatch("", ANNOTE, 0, stmt))
				{
				Function fun = act.getFunction();
				stmts.insertChild(argstmt, s+bcount);
				bcount += 1;
				stmts.insertChild(repstmt, s+bcount);
				bcount += 1;
				for (Stmt fstmt : fun.getStmts()){
						stmts.insertChild(fstmt, s+bcount+tcount);
						bcount += 1;
				}			
			}
		}
	 ...
	 * s: index of assignment statement (child) in statement list
	 */
	private static int setMatchAndWeave(ast.List<Stmt> stmts, int s, String varName, AssignStmt context) {
		...
		int acount = 0, bcount = 0, tcount= 0; //acount number of after actions, bcount number of before actions, tcount number of transformations
		boolean aroundExist = false;
		Expr prevCase = null;
		...
		while (st.hasMoreTokens()) {
			...
			for(int j=0; j<actionsList.size(); j++)
			{
				ActionInfo act = actionsList.get(j);
				 ...
				 if(patternsList.containsKey(act.getPattern()) 
						&& patternsList.get(act.getPattern()).ShadowMatch(target, SET, args, context)
				){
					Function fun = act.getFunction();
	...
	private static int getOrCallMatchAndWeave(ast.List<Stmt> stmts, int s, String target, AssignStmt context, VFDatum varOrFun, boolean isScript) {
		...
		for(int j=0; j<actionsList.size(); j++)
		{
			...
			if(patternsList.containsKey(act.getPattern())) {
				Expr pat = patternsList.get(act.getPattern());
	...
	private static int exprMatchAndWeave(ast.List<Stmt> stmts, int s, String target, ExprStmt context, VFDatum varOrFun, boolean isScript) {
		...
		for(int j=0; j<actionsList.size(); j++)
		{
			...
			boolean isGet = false, isCall = false, isOp = false, isType = false;
			...
			if(patternsList.containsKey(act.getPattern())) {
				...
				if(varOrFun.isVariable()) { //only match if target is variable
					...
					isGet = pat.ShadowMatch(target, GET, args, context);
					isType = pat.ShadowMatch(target, TYPE, args, context);
					pattern = GET;
	...
	public static void weaveFunction(Function func)
	{
		...
		for(int j=0; j<actionsList.size(); j++)
		{
			ActionInfo act = actionsList.get(j);
			if(patternsList.containsKey(act.getPattern()) 
					&& patternsList.get(act.getPattern()).ShadowMatch(func.getName(), EXECUTION, func.getInputParams().getNumChild(), func)
			){
	...
	public static void weaveScript(Script script)
	{
		...
		for(int j=0; j<actionsList.size(); j++)
		{
			...
			if(patternsList.containsKey(act.getPattern()) 
					&& patternsList.get(act.getPattern()).ShadowMatch(name, EXECUTION, 0, script)
			){
	...
	public static void weaveMain(Program p, ast.List<Stmt> lstbefore, ast.List<Stmt> lstafter) {
		...
		for(int j=0; j<actionsList.size(); j++)
		{
			...
			if(patternsList.containsKey(act.getPattern()) 
					&& patternsList.get(act.getPattern()).ShadowMatch("", MAINEXECUTION, 0, p)
			){
